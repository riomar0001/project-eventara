  ...
  return (
    ...
    >
      ...
      >
        {/* Close button */}
        <button
          className="absolute top-[14px] right-[14px] z-[3] grid h-8 w-8 place-items-center rounded-[10px] transition-all"
          style={{
            color: 'var(--text-mute)',
          ...
          onClick={onClose}
          onMouseEnter={(e) => {
            (e.currentTarget as HTMLElement).style.background = 'oklch(0 0 0 / 0.6)';
        ...
        <div
          ...
          style={{
            aspectRatio: '16/7',
            background: 'linear-gradient(135deg, oklch(0.22 0.012 150), oklch(0.15 0.008 150))'
          }}
        >
          <div
          ...
          />
          <div
            className="absolute top-[20%] left-[18%] h-[220px] w-[220px] rounded-full opacity-50 blur-[38px]"
            style={{
              background: ev.orb === 'lime' ? 'var(--lime)' : 'var(--amber)'
            }}
          />
          <div
            className="absolute right-[15%] bottom-[10%] h-[160px] w-[160px] rounded-full opacity-40 blur-[32px]"
            style={{
              background: ev.orb === 'lime' ? 'var(--amber)' : 'var(--lime)'
            }}
          />
          <div
            className="absolute top-4 left-5 z-[2] min-w-[54px] rounded-[10px] border px-[10px] py-2 text-center"
            style={{
          ...
          >
            <div className="font-mono text-[10px] font-medium tracking-[0.14em]" style={{ color: 'var(--lime)' }}>
              {ev.mo}
            </div>
            <div className="mt-px text-[20px] leading-none font-bold tracking-[-0.03em]" style={{ color: 'var(--text)' }}>
              {ev.day}
            </div>
          </div>
        </div>
        ...
        <div className="px-7 pt-6 pb-7">
          ...
          <h3 className="m-0 mb-2 text-[22px] font-semibold tracking-[-0.025em]" style={{ color: 'var(--text)' }}>
            {ev.title}
          </h3>

          <p className="m-0 mb-[18px] text-[14.5px] leading-[1.6]" style={{ color: 'var(--text-dim)' }}>
            {ev.desc}
          </p>
          ...
          <div className="mb-[18px] grid grid-cols-2 gap-3">
            ...
            >
              <div className="font-mono text-[10.5px] tracking-[0.14em] uppercase" style={{ color: 'var(--text-mute)' }}>
                Date
              </div>
            ...
            >
              ...
              <div
                ...
                style={{
                  color: ev.seats === 0 ? 'var(--amber)' : 'var(--lime)'
                }}
              >
                {ev.seats === 0 ? 'Full — waitlist' : `${ev.seats} of ${ev.total} left`}
              </div>
            </div>
          </div>
