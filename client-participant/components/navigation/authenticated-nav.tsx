...
export function AuthenticatedNav({ userName = 'User', userTier = 'Participant', activeLink = 'Venue Hub', onLogout }: AuthenticatedNavProps) {
      ...
      <div className="mx-auto max-w-[1240px] px-8">
        <div className="grid h-[68px] grid-cols-[1fr_auto_1fr] items-center gap-6">
          ...
          <div className="flex items-center gap-2.5 font-bold tracking-[-0.02em]">
            <div className="flex h-6.5 w-6.5 items-center justify-center rounded-lg bg-gradient-to-br from-[var(--lime)] to-[oklch(0.78_0.19_128)] shadow-[var(--lime-glow)] shadow-lg">
              <svg width="16" height="16" viewBox="0 0 20 20" fill="none">
                <path d="M10 2L15 8H12V14H8V8H5L10 2Z" fill="currentColor" className="text-[#0a1005]" />
              </svg>
            </div>
            ...
            {navLinks.map((link) => (
              <button
                ...
                className={`rounded-xl px-3.5 py-2 text-[14.5px] transition-colors ${
                  link === activeLink
                    ? 'relative text-[var(--lime)] after:absolute after:bottom-0 after:left-1/2 after:h-px after:w-8 after:-translate-x-1/2 after:bg-[var(--lime)] after:shadow-[var(--lime-glow)] after:shadow-lg'
            ...
            <div className="relative">
              ...
              >
                ...
                <div className="hidden flex-col items-start sm:flex">
                  <div className="text-xs font-semibold text-[var(--text)]">{userName}</div>
                  <div className="font-mono text-xs font-medium text-[var(--lime)]">{userTier}</div>
                </div>
              </button>

              {/* Avatar Dropdown */}
              {avatarMenuOpen && (
                <div className="absolute top-full right-0 z-50 mt-2 w-48 rounded-lg border border-[var(--line)] bg-[var(--surface)] shadow-xl">
