...
export function VenueCard({ venue, onView, onEdit, onShare, onReport }: VenueCardProps) {
  ...
  return (
    <div className="flex h-[580px] flex-col overflow-hidden rounded-2xl border border-[var(--line-soft)] bg-[var(--surface)] transition-all duration-280 hover:border-[oklch(0.9_0.22_128_/_0.45)] hover:shadow-lg hover:shadow-[oklch(0.9_0.22_128_/_0.15)]">
      {/* Visual Section */}
      <div className="relative aspect-video overflow-hidden bg-gradient-to-br from-[oklch(0.2_0.01_150)] to-[oklch(0.15_0.008_150)]">
        ...
        {/* Placeholder label */}
        <div className="absolute bottom-4 left-4 font-mono text-xs tracking-widest text-[oklch(1_0_0_/_0.3)] uppercase">[ venue cover · 16:9 ]</div>
      </div>
      ...
      <div className="flex flex-1 flex-col gap-2.5 p-5">
        ...
        <div className="flex items-start justify-between gap-2">
          <h3 className="m-0 text-lg font-semibold tracking-[-0.02em] text-[var(--text)]">{venue.name}</h3>
        ...
        <div className="text-xs text-[var(--text-dim)]">
          ...
          <div className="text-[var(--text-mute)]">{formatDate(venue.contributor.date)}</div>
        </div>
        ...
        {venue.tags.length > 0 && (
          <div className="flex flex-wrap gap-1.5 py-1.5">
            {venue.tags.map((tag) => (
              <span key={tag} className="rounded-md bg-[oklch(0_0_0_/_0.025)] px-2 py-1 text-xs font-medium text-[var(--text-dim)]">
                {tag}
              </span>
            ))}
          </div>
        ...
        <div className="border-t border-dashed border-[var(--line-soft)] pt-2.5">
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex-1 rounded-full border border-[var(--lime)] bg-[oklch(0.9_0.22_128_/_0.05)] px-3 py-2 text-sm font-medium text-[var(--lime)] transition-all hover:bg-[oklch(0.9_0.22_128_/_0.12)] hover:shadow-[var(--lime-glow)] hover:shadow-lg"
            >
              View
            </button>
            ...
            <div className="relative">
              ...
              {/* Dropdown menu */}
              {isMenuOpen && (
                <div className="absolute top-full right-0 z-10 mt-2 w-40 rounded-lg border border-[var(--line)] bg-[var(--surface)] shadow-lg">
                  <button
                    onClick={() => {
                      onEdit(venue);
                      setIsMenuOpen(false);
