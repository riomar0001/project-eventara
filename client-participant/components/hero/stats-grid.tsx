  ...
  return (
    <div className="grid grid-cols-4 border-t border-b border-[var(--line-soft)]">
      {stats.map((stat, index) => (
        <div key={index} className={`px-[18px] py-[22px] text-left ${index < stats.length - 1 ? 'border-r border-[var(--line-soft)]' : ''}`}>
          <div
            className={`text-[28px] font-semibold tracking-[-0.03em] ${
              stat.color === 'lime' ? 'text-[var(--lime)]' : stat.color === 'amber' ? 'text-[var(--amber)]' : 'text-[var(--text)]'
            }`}
          >
            {stat.value}
          </div>
          <div className="mt-1 font-mono text-[11px] tracking-[0.16em] text-[var(--text-mute)] uppercase">{stat.label}</div>
        </div>
      ))}
    </div>
  );
