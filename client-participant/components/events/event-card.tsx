...
export function EventCard({ event, onClick }: EventCardProps) {
  return (
    <article
      onClick={onClick}
      className="ease flex cursor-pointer flex-col overflow-hidden rounded-[20px] border border-line-soft bg-surface transition-[transform_280ms_ease,border-color_280ms_ease,box-shadow_280ms_ease] hover:-translate-y-[6px] hover:border-[oklch(0.7_0.2_130_/_0.5)] hover:shadow-[0_20px_60px_-20px_oklch(0.7_0.2_130_/_0.3),0_0_0_1px_oklch(0.7_0.2_130_/_0.1)]"
    >
      {/* Event visual */}
      <div
        className="relative aspect-[16/9] overflow-hidden border-b border-line-soft"
        style={{
      ...
      >
        {/* Orb decorations */}
        <div
          className={`absolute left-[20%] top-[30%] h-[160px] w-[160px] rounded-full opacity-55 blur-[34px] ${event.orbColor === 'lime' ? 'bg-lime' : 'bg-amber'}`}
        />
        <div
        ...
        {/* Chip badge */}
        <div
          className="absolute top-3 left-3 rounded-md border border-[oklch(1_0_0/0.1)] bg-black/40 px-[9px] py-[5px] font-mono text-[10.5px] tracking-[0.12em] text-white uppercase backdrop-blur-md"
        >
          {event.chip}
        </div>

        {/* Label */}
        <div className="absolute bottom-3 left-3 font-mono text-[9.5px] tracking-[0.15em] uppercase text-white/30">
          [ event cover · 16:9 ]
        </div>
      </div>
