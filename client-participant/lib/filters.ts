...
export const filterVenuesByCapacity = (venues: Venue[], capacityKey: string): Venue[] => {
...
export const filterVenuesBySearch = (venues: Venue[], query: string): Venue[] => {
  if (!query.trim()) return venues;

  const lowerQuery = query.toLowerCase();
  return venues.filter(
    (v) =>
      v.name.toLowerCase().includes(lowerQuery) || v.location.toLowerCase().includes(lowerQuery) || v.tags.some((tag) => tag.toLowerCase().includes(lowerQuery))
  );
};
...
export const applyFilters = (venues: Venue[], query: string, capacityKey: string, sortKey: string): Venue[] => {
  let filtered = venues;
...
export const paginate = <T>(items: T[], page: number): { items: T[]; totalPages: number } => {
  // Page 1 shows 6 items (3 partnered + 3 community), page 2+ shows 3 per page
  if (page === 1) {
    const firstPageItems = 6;
    const remainingPerPage = 3;
    const totalPages = Math.ceil((items.length - firstPageItems) / remainingPerPage) + 1;
    ...
    const totalPages = Math.ceil((items.length - firstPageItems) / itemsPerPage) + 1;
