    ...
    <main style={{ background: 'var(--bg)', color: 'var(--text)' }}>
      ...
      <div style={{ position: 'relative' }}>
        ...
        <div className="pointer-events-none absolute inset-x-0 top-0 z-0 h-[620px] overflow-hidden" aria-hidden="true">
          <div
            className="absolute top-[-220px] left-[-180px] h-[640px] w-[640px] rounded-full blur-[90px]"
            style={{
              background: 'radial-gradient(circle, oklch(0.9 0.22 128 / 0.45), transparent 65%)'
            }}
          />
          <div
            className="absolute top-[-120px] right-[-140px] h-[540px] w-[540px] rounded-full blur-[90px]"
            style={{
        ...
        <div className="events-dir-container relative z-[1] container mx-auto max-w-[1240px] px-8">
          {/* Page header */}
          <header className="relative pt-16 pb-9">
            <div className="flex flex-wrap items-start justify-between gap-5">
              <div>
                ...
                <h1
                  className="mx-0 my-[18px] leading-none font-bold tracking-[-0.035em]"
                  style={{
                ...
                <p className="m-0 mb-9 max-w-[58ch] text-[16px] leading-[1.55]" style={{ color: 'var(--text-dim)' }}>
                  Browse {ALL_EVENTS.length} upcoming workshops, meetups and hackathons across the Davao DeFi community.
                </p>
              </div>
              <div className="text-right max-sm:text-left">
                <div className="font-mono text-[40px] leading-none font-semibold tracking-[-0.03em]" style={{ color: 'var(--lime)' }}>
                  {String(ALL_EVENTS.length).padStart(2, '0')}
                </div>
                <div className="mt-[2px] font-mono text-[11px] tracking-[0.18em] uppercase" style={{ color: 'var(--text-mute)' }}>
                  Active listings
                </div>
              </div>
            </div>
          ...
          <section>
            ...
            <div className="mt-14 mb-[22px] flex flex-wrap items-end justify-between gap-6">
              <div>
                <h3 className="m-0 text-[24px] font-semibold tracking-[-0.02em]" style={{ color: 'var(--text)' }}>
                  {cat === 'All' ? 'All events' : cat}
                </h3>
                <div className="mt-1 font-mono text-[12px]" style={{ color: 'var(--text-mute)' }}>
                  {filtered.length} result
                  {filtered.length !== 1 ? 's' : ''}
                  {q && (
                    <>
                      {' '}
                      · query <span style={{ color: 'var(--lime)' }}>&quot;{q}&quot;</span>
                    </>
                  )}
                </div>
              </div>
              <div className="flex gap-[2px] rounded-[10px] border p-[3px]" style={{ borderColor: 'var(--line-soft)' }}>
                <button
                  ...
                  style={{
                    ...
                    color: view === 'grid' ? 'var(--text)' : 'var(--text-mute)'
                  }}
            ...
            <div
              ...
              style={{
                gridTemplateColumns: view === 'list' ? '1fr' : tweaks.density === 'compact' ? 'repeat(4, 1fr)' : 'repeat(3, 1fr)'
              }}
            >
              {paged.length === 0 ? (
                <div
                ...
                >
                  ...
                  <h4 className="m-0 mb-[6px] text-[20px] font-semibold tracking-[-0.02em]" style={{ color: 'var(--text)' }}>
      ...
      {editMode && (
        <div
          className="fixed right-5 bottom-5 z-80 w-[280px] rounded-[16px] border p-4 text-[13px] shadow-lg"
          style={{
        ...
        >
          <h4 className="m-0 mb-3 flex items-center gap-2 text-[13px] font-semibold">
            <span className="h-[6px] w-[6px] rounded-full" style={{ background: 'var(--lime)' }} />
            Tweaks
            <button className="ml-auto" style={{ color: 'var(--text-mute)' }} onClick={() => setEditMode(false)}>
              <Icon name="x" size={14} />
            </button>
          </h4>
          <div className="flex items-center justify-between gap-[10px] border-t py-2" style={{ borderColor: 'var(--line-soft)' }}>
            ...
            <select
              ...
              onChange={(e) => setTweaks({ ...tweaks, density: e.target.value as TweaksConfig['density'] })}
            >
          ...
          <div className="flex items-center justify-between gap-[10px] border-t py-2" style={{ borderColor: 'var(--line-soft)' }}>
            ...
            <select
              ...
              onChange={(e) => setTweaks({ ...tweaks, bannerVariant: e.target.value as TweaksConfig['bannerVariant'] })}
            >
          ...
          <div className="flex items-center justify-between gap-[10px] border-t py-2" style={{ borderColor: 'var(--line-soft)' }}>
            ...
            <select
              ...
              onChange={(e) => setTweaks({ ...tweaks, badgeStyle: e.target.value as TweaksConfig['badgeStyle'] })}
            >
