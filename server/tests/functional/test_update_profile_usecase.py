"""Functional test cases for UpdateProfileUseCase."""
...
class TestUpdateProfileSuccess:
    ...
    async def test_commits_and_returns_updated_profile(self):
        """Commits the transaction and returns the updated profile on success"""
    ...
    async def test_previous_profile_is_pre_update_snapshot(self):
        """Includes the pre-update profile snapshot in the output for audit log construction"""
    ...
    async def test_skips_alias_check_when_alias_unchanged(self):
        """Does not query alias availability when the submitted alias matches the current one"""
    ...
    async def test_checks_alias_uniqueness_when_alias_changes(self):
        """Queries alias availability when the submitted alias differs from the current one"""
...
class TestUpdateProfileGuards:
    ...
    async def test_raises_user_inactive_when_account_is_inactive(self):
        """Raises UserInactiveError when the account status is INACTIVE"""
    ...
    async def test_raises_user_inactive_when_account_is_deleted(self):
        """Raises UserInactiveError when the account status is DELETED"""
    ...
    async def test_raises_profile_not_found_when_no_profile_exists(self):
        """Raises ProfileNotFoundError when the user has no profile row (onboarding incomplete)"""
        repo = _make_repo()
    ...
    async def test_raises_alias_taken_when_new_alias_owned_by_other(self):
        """Raises AliasAlreadyTakenError when the new alias is already owned by another account"""
    ...
    async def test_integrity_error_raises_alias_taken_and_rolls_back(self):
        """Raises AliasAlreadyTakenError and rolls back when the database unique constraint fires"""
    ...
    async def test_repo_update_returns_none_raises_profile_not_found_and_rolls_back(self):
        """Raises ProfileNotFoundError and rolls back when the repository update matches no row"""
        repo = _make_repo(profile=_make_profile())
