"""Functional test cases for EventParticipantUseCase (register_for_session and update_participant_status)."""
...
async def test_register_raises_session_not_found_when_session_missing():
    """Raises EventSessionNotFoundError when the target session does not exist."""
...
async def test_register_raises_registration_not_open_when_session_is_draft():
    """Raises RegistrationNotOpenError when session status is DRAFT."""
    event_repo, participant_repo = _make_register_repos()
...
async def test_register_raises_registration_not_open_when_session_is_started():
    """Raises RegistrationNotOpenError when session status is STARTED."""
    event_repo, participant_repo = _make_register_repos()
...
async def test_register_raises_registration_not_open_when_session_is_cancelled():
    """Raises RegistrationNotOpenError when session status is CANCELLED."""
    event_repo, participant_repo = _make_register_repos()
...
async def test_register_raises_duplicate_when_user_already_registered():
    """Raises DuplicateEventParticipantError when user is already registered for the session."""
    event_repo, participant_repo = _make_register_repos()
...
@pytest.mark.asyncio
async def test_register_acquires_session_lock_to_serialise_concurrent_registrations():
    """Fetches session with FOR UPDATE lock to prevent concurrent registration races."""
...
@pytest.mark.asyncio
async def test_register_creates_participant_with_correct_user_and_session():
    """Calls create with the authenticated user's ID and the target session ID."""
...
@pytest.mark.asyncio
async def test_register_commits_transaction_on_success():
    """Commits the database transaction after successful registration."""
...
@pytest.mark.asyncio
async def test_register_rolls_back_on_repo_failure():
    """Rolls back the transaction when the repository create operation raises."""
    event_repo, participant_repo = _make_register_repos()
...
async def test_register_does_not_commit_when_session_not_found():
    """Does not commit when the session does not exist."""
    event_repo, participant_repo = _make_register_repos()
    event_repo.get_session_by_id = AsyncMock(return_value=None)
    uc, _, _, db = _make_register_uc(event_repo, participant_repo)
    with pytest.raises(EventSessionNotFoundError):
...
@pytest.mark.asyncio
async def test_register_returns_participant_with_registered_status():
    """Returns the newly created participant record with REGISTERED status."""
...
async def test_get_participants_allowed_for_event_organizer():
    """Returns participants without checking volunteer status when caller is organizer."""
...
async def test_get_participants_allowed_for_joined_volunteer():
    """Returns participants when caller has a JOINED volunteer assignment."""
    event_repo = MagicMock(spec=EventRepository)
...
async def test_get_participants_raises_for_non_joined_volunteer():
    """Raises UnauthorizedEventParticipantOperationError when caller lacks event access."""
    event_repo = MagicMock(spec=EventRepository)
    event_repo.get_event_by_id = AsyncMock(return_value=_sample_event(created_by=CREATOR_ID))
    uc, _, _, _ = _make_participant_access_uc(event_repo=event_repo)
...
async def test_get_participants_passes_status_filter_to_repo():
    """Forwards status, limit, and offset to the participant repository."""
...
async def test_register_raises_slots_full_when_session_max_slots_exhausted():
    """Raises SessionSlotsFullError when active participants equal session max_slots."""
    event_repo, participant_repo = _make_register_repos()
...
async def test_register_raises_slots_full_when_venue_capacity_exhausted():
    """Raises SessionSlotsFullError when active participants equal venue capacity (no session override)."""
    event_repo, participant_repo = _make_register_repos()
...
async def test_register_uses_session_max_slots_over_venue_capacity():
    """Raises SessionSlotsFullError at session max_slots even when venue has more capacity."""
    event_repo, participant_repo = _make_register_repos()
...
@pytest.mark.asyncio
async def test_register_succeeds_when_slot_count_is_below_max_slots():
    """Allows registration when active participant count is below the effective capacity."""
    event_repo, participant_repo = _make_register_repos()
    event_repo.get_session_by_id = AsyncMock(return_value=_sample_session(max_slots=50))
...
async def test_register_fetches_venue_capacity_when_session_max_slots_is_none():
    """Falls back to venue capacity lookup when session max_slots is not set."""
    event_repo, participant_repo = _make_register_repos()
    event_repo.get_session_by_id = AsyncMock(return_value=_sample_session(max_slots=None))
...
async def test_register_skips_venue_capacity_lookup_when_session_max_slots_set():
    """Does not call get_venue_capacity when session max_slots is explicitly set."""
    event_repo, participant_repo = _make_register_repos()
    event_repo.get_session_by_id = AsyncMock(return_value=_sample_session(max_slots=50))
    uc, event_repo_used, _, _ = _make_register_uc(event_repo, participant_repo)
    await uc.register_for_session(_register_input())
...
@pytest.mark.asyncio
async def test_update_raises_participant_not_found_when_record_missing():
    """Raises EventParticipantNotFoundError when the participant record does not exist."""
...
async def test_update_raises_session_not_found_when_session_missing():
    """Raises EventSessionNotFoundError when the participant's session no longer exists."""
    event_repo, participant_repo = _make_update_repos()
    event_repo.get_session_by_id = AsyncMock(return_value=None)
    uc, _, _, _ = _make_update_uc(event_repo, participant_repo)
    with pytest.raises(EventSessionNotFoundError):
...
@pytest.mark.asyncio
async def test_update_raises_event_not_found_when_parent_event_missing():
    """Raises EventNotFoundError when the parent event no longer exists."""
    event_repo, participant_repo = _make_update_repos()
    event_repo.get_event_by_id = AsyncMock(return_value=None)
    uc, _, _, _ = _make_update_uc(event_repo, participant_repo)
    with pytest.raises(EventNotFoundError):
...
@pytest.mark.asyncio
async def test_update_raises_unauthorized_when_caller_is_not_event_creator():
    """Raises UnauthorizedEventParticipantOperationError when caller is not the event creator."""
...
@pytest.mark.asyncio
async def test_update_raises_transition_error_when_transitioning_from_attended():
    """Raises InvalidEventParticipantStatusTransitionError when participant is already ATTENDED."""
    event_repo, participant_repo = _make_update_repos()
...
@pytest.mark.asyncio
async def test_update_raises_transition_error_when_transitioning_from_no_show():
    """Raises InvalidEventParticipantStatusTransitionError when participant status is NO_SHOW."""
    event_repo, participant_repo = _make_update_repos()
...
@pytest.mark.asyncio
async def test_update_raises_transition_error_when_transitioning_from_cancelled():
    """Raises InvalidEventParticipantStatusTransitionError when participant status is CANCELLED."""
    event_repo, participant_repo = _make_update_repos()
...
@pytest.mark.asyncio
async def test_update_raises_participant_not_found_when_concurrent_delete_wins():
    """Raises EventParticipantNotFoundError when update_status returns None due to a concurrent delete."""
    event_repo, participant_repo = _make_update_repos()
...
@pytest.mark.asyncio
async def test_update_acquires_participant_row_lock_before_read():
    """Fetches participant with FOR UPDATE lock to serialise concurrent status updates."""
...
async def test_update_calls_update_status_with_correct_args():
    """Calls update_status with the target participant ID and the requested new status."""
    uc, _, participant_repo, _ = _make_update_uc()
    await uc.update_participant_status(_update_input())
...
@pytest.mark.asyncio
async def test_update_commits_transaction_on_success():
    """Commits the database transaction after a successful status update."""
...
@pytest.mark.asyncio
async def test_update_rolls_back_on_repo_failure():
    """Rolls back the transaction when the repository update operation raises."""
    event_repo, participant_repo = _make_update_repos()
...
@pytest.mark.asyncio
async def test_update_returns_updated_participant_and_pre_update_snapshot():
    """Returns both the updated participant and the original snapshot for audit logging."""
    uc, _, _, _ = _make_update_uc()
