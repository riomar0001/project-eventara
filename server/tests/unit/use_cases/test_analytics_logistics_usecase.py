...
def _make_volunteer_logistics(**overrides):
    ...
    defaults = {
        "event_id": EVENT_ID,
        "event_title": "Test Event",
        "joined_volunteer_count": 1,
        "joined_volunteer_roster": [roster],
        "volunteer_to_participant_ratio": 0.5,
        "pending_volunteer_count": 2,
    }
...
class TestGetEventLogisticsOverview:
    ...
    async def test_returns_overview_for_valid_event(self):
        result = await _make_uc().get_event_logistics_overview(GetEventLogisticsInput(event_id=EVENT_ID))
    ...
    async def test_raises_event_not_found_when_event_does_not_exist(self):
        repo = _make_repo(get_event_logistics_overview=AsyncMock(side_effect=EventNotFoundError(f"Event {EVENT_ID} not found")))
        with pytest.raises(EventNotFoundError):
            await _make_uc(repo).get_event_logistics_overview(GetEventLogisticsInput(event_id=EVENT_ID))
        ...
        repo = _make_repo(
            get_event_logistics_overview=AsyncMock(
                return_value=_make_overview(
                    total_sessions=0, scheduled_dates=[], venue_assignments=[], session_utilisation=[], venue_capacity_vs_registrations=[]
                )
            )
        )
        result = await _make_uc(repo).get_event_logistics_overview(GetEventLogisticsInput(event_id=EVENT_ID))
    ...
    async def test_returns_over_capacity_flag(self):
        ...
        repo = _make_repo(get_event_logistics_overview=AsyncMock(return_value=_make_overview(over_capacity_sessions=[util])))
        result = await _make_uc(repo).get_event_logistics_overview(GetEventLogisticsInput(event_id=EVENT_ID))
    ...
    async def test_utilisation_100_percent(self):
        ...
        repo = _make_repo(get_event_logistics_overview=AsyncMock(return_value=_make_overview(session_utilisation=[util])))
        result = await _make_uc(repo).get_event_logistics_overview(GetEventLogisticsInput(event_id=EVENT_ID))
    ...
    async def test_raises_analytics_data_fetch_error_on_repo_failure(self):
        repo = _make_repo(get_event_logistics_overview=AsyncMock(side_effect=RuntimeError("db down")))
...
class TestGetSessionTimeline:
    ...
    async def test_empty_timeline_returns_empty_lists(self):
        repo = _make_repo(get_session_timeline=AsyncMock(return_value=SessionTimeline(ongoing=[], upcoming=[], completed=[])))
    ...
    @pytest.mark.asyncio
    async def test_raises_analytics_data_fetch_error_on_repo_failure(self):
        repo = _make_repo(get_session_timeline=AsyncMock(side_effect=RuntimeError("db down")))
        with patch("app.core.config.settings") as s:
            s.DEBUG = False
            with pytest.raises(AnalyticsDataFetchError):
...
class TestGetVolunteerLogistics:
    ...
    async def test_returns_volunteer_logistics_for_valid_event(self):
        result = await _make_uc().get_volunteer_logistics(GetVolunteerLogisticsInput(event_id=EVENT_ID))
    ...
    async def test_raises_event_not_found(self):
        repo = _make_repo(get_volunteer_logistics=AsyncMock(side_effect=EventNotFoundError(f"Event {EVENT_ID} not found")))
        with pytest.raises(EventNotFoundError):
            await _make_uc(repo).get_volunteer_logistics(GetVolunteerLogisticsInput(event_id=EVENT_ID))
        ...
        repo = _make_repo(
            get_volunteer_logistics=AsyncMock(
                return_value=_make_volunteer_logistics(
                    joined_volunteer_count=0,
                    joined_volunteer_roster=[],
                    volunteer_to_participant_ratio=None,
                    pending_volunteer_count=0,
                )
            )
        )
        result = await _make_uc(repo).get_volunteer_logistics(GetVolunteerLogisticsInput(event_id=EVENT_ID))
    ...
    async def test_ratio_none_when_no_checkins(self):
        repo = _make_repo(get_volunteer_logistics=AsyncMock(return_value=_make_volunteer_logistics(volunteer_to_participant_ratio=None)))
        result = await _make_uc(repo).get_volunteer_logistics(GetVolunteerLogisticsInput(event_id=EVENT_ID))
        assert result.logistics.volunteer_to_participant_ratio is None
...
class TestGetRegistrationLogistics:
    ...
    async def test_returns_registration_logistics_for_valid_event(self):
        result = await _make_uc().get_registration_logistics(GetRegistrationLogisticsInput(event_id=EVENT_ID))
    ...
    async def test_empty_event_returns_empty_list(self):
        ...
        result = await _make_uc(repo).get_registration_logistics(GetRegistrationLogisticsInput(event_id=EVENT_ID))
        assert result.registrations == []

    @pytest.mark.asyncio
    async def test_zero_registrations_returns_zero_rates(self):
        repo = _make_repo(
            get_registration_logistics=AsyncMock(
                return_value=[
                    _make_registration_logistics(
                        ...
                        no_show_count=0,
                        no_show_rate_pct=None,
                        qr_checkin_count=0,
                        manual_checkin_count=0,
                    )
                ]
            )
        )
    ...
    @pytest.mark.asyncio
    async def test_raises_analytics_data_fetch_error_on_repo_failure(self):
        repo = _make_repo(get_registration_logistics=AsyncMock(side_effect=RuntimeError("db down")))
        with patch("app.core.config.settings") as s:
            s.DEBUG = False
            with pytest.raises(AnalyticsDataFetchError):
                await _make_uc(repo).get_registration_logistics(GetRegistrationLogisticsInput(event_id=EVENT_ID))
