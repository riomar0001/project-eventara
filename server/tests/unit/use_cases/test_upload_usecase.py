...
def _make_event_uc(repo=None) -> EventUseCase:
    return EventUseCase(repo=repo or _make_event_repo(event=_make_event()), db=AsyncMock())
...
def _make_venue_uc(repo=None) -> VenueManagementUseCase:
    return VenueManagementUseCase(repo=repo or _make_venue_repo(venue=_make_venue()), db=AsyncMock())
...
def _make_profile_uc(repo=None) -> UpdateProfileAvatarUseCase:
    return UpdateProfileAvatarUseCase(repo=repo or _make_profile_repo(), db=AsyncMock())
