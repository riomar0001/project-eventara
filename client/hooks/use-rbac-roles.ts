...
import { RbacRoles } from '@/api/sdk.gen';
...
export function useRbacRoles() {
  ...
  useEffect(() => {
    ...
    async function loadRoles() {
      ...
      try {
        const result = await RbacRoles.listRolesRolesGet({
          headers: getRequestHeaders(),
          throwOnError: false
        });

        if (!result.data) {
          throw result.error ?? new Error('Unable to load RBAC roles right now.');
        }
        ...
        if (!cancelled) {
          setRoles([]);
          setError(getRbacErrorMessage(nextError, 'Unable to load RBAC roles right now.'));
        }
      } finally {
        if (!cancelled) {
          setIsLoading(false);
  ...
  async function createRole(input: RoleDraft) {
    ...
    try {
      const result = await RbacRoles.createRoleRolesPost({
        body: input,
        headers: getRequestHeaders(),
        throwOnError: false
      });
      ...
      toast.success(result.data.message ?? RBAC_COPY.roles.savedCreate);
      refresh();
      return result.data.data;
    } catch (nextError) {
      toast.error(getRbacErrorMessage(nextError, 'Unable to create the role right now.'));
      return null;
    } finally {
      setIsSaving(false);
    }
  ...
  async function updateRole(roleId: string, input: RoleUpdateRequest) {
    ...
    try {
      const result = await RbacRoles.updateRoleRolesRoleIdPatch({
        body: input,
        headers: getRequestHeaders(),
        path: { role_id: roleId },
        throwOnError: false
      ...
      toast.success(result.data.message ?? RBAC_COPY.roles.savedUpdate);
      refresh();
      return result.data.data;
    } catch (nextError) {
      toast.error(getRbacErrorMessage(nextError, 'Unable to update the role right now.'));
      return null;
    } finally {
      setIsSaving(false);
    }
  ...
  async function deleteRole(roleId: string) {
    ...
    try {
      const result = await RbacRoles.deleteRoleRolesRoleIdDelete({
        headers: getRequestHeaders(),
        path: { role_id: roleId },
        throwOnError: false
      });
      ...
      toast.success(RBAC_COPY.roles.savedDelete);
      refresh();
      return true;
    } catch (nextError) {
      toast.error(getRbacErrorMessage(nextError, 'Unable to delete the role right now.'));
      return false;
    } finally {
      setIsDeleting(false);
    }
