...
import {
  ...
  Download, Eye, Filter, MoreHorizontal, Pencil, Copy, Trash2,
} from 'lucide-react';
...
const transactions: Transaction[] = [
  ...
  { id: '2', date: 'Apr 8, 2026',  description: 'Salary deposit',        initials: 'AC', category: 'Income',        amount: 5200.00, status: 'completed' },
  { id: '3', date: 'Apr 7, 2026',  description: 'Grocery store',         initials: 'WM', category: 'Food',          amount: -84.32,  status: 'completed' },
  { id: '4', date: 'Apr 6, 2026',  description: 'Electric bill',         initials: 'EL', category: 'Housing',       amount: -112.00, status: 'pending'   },
  { id: '5', date: 'Apr 5, 2026',  description: 'Transfer to savings',   initials: 'BK', category: 'Savings',       amount: -500.00, status: 'completed' },
  { id: '6', date: 'Apr 4, 2026',  description: 'Gym membership',        initials: 'GM', category: 'Healthcare',    amount: -49.00,  status: 'failed'    },
  { id: '7', date: 'Apr 3, 2026',  description: 'Freelance payment',     initials: 'CL', category: 'Income',        amount: 750.00,  status: 'completed' },
  { id: '8', date: 'Apr 2, 2026',  description: 'Uber ride',             initials: 'UB', category: 'Transport',     amount: -23.50,  status: 'completed' },
];
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between pb-3">
          <div>
            ...
            <CardDescription>{filtered.length} of {transactions.length} records</CardDescription>
          </div>
          <CardAction>
                ...
                <DropdownMenuTrigger asChild>
                  <Button variant="outline" size="sm" className="gap-1.5">
                    ...
                    {statusFilter.length > 0 && (
                      <Badge className="ml-0.5 h-4 px-1 text-[10px]">{statusFilter.length}</Badge>
                    )}
                  </Button>
                </DropdownMenuTrigger>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  <tr key={t.id} className={`border-b transition-colors hover:bg-muted/30 ${i % 2 !== 0 ? 'bg-muted/10' : ''}`}>
                    <td className="whitespace-nowrap px-6 py-3.5 text-xs text-muted-foreground">{t.date}</td>
                    <td className="px-6 py-3.5">
                      <div className="flex items-center gap-2.5">
                        <Avatar size="sm"><AvatarFallback className="text-[9px]">{t.initials}</AvatarFallback></Avatar>
