          ...
          {transactions.map((tx) => (
            <div key={tx.id} className="flex items-center gap-2.5">
              ...
              <div className="flex flex-col items-end shrink-0">
                <span
                  className={cn("text-xs font-semibold", tx.amount.startsWith("+") ? "text-green-600" : "text-foreground")}
                >
                  {tx.amount}
                </span>
                <Badge
                  ...
                  className={cn(
                    "h-4 px-1 text-[9px]",
                    tx.status === "Completed" ? "bg-green-50 text-green-700" : "bg-red-50 text-red-600"
                  )}
                >
                  {tx.status}
                </Badge>
