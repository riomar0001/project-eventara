...
function DetailRow({ icon: Icon, label, value }: { icon: React.ComponentType<{ className?: string }>; label: string; value: string }) {
  return (
    <div className="rounded-xl border-neutral-200 bg-white px-3 py-2.5">
      <div className="flex items-center gap-2 text-[11px] font-semibold tracking-[0.18em] text-neutral-500 uppercase">
        <Icon className="size-3" />
        {label}
      </div>
      <p className="mt-1.5 text-[13px] leading-5 font-medium break-all text-neutral-950">{value}</p>
    </div>
  );
}
      ...
      <Card className="border-0 bg-white shadow-none ring-1 ring-neutral-200 xl:sticky xl:top-6 xl:flex xl:h-190 xl:min-h-190 xl:flex-col">
        ...
        <CardContent className="flex min-h-64 items-center justify-center px-6 py-10 xl:flex-1">
          <div className="max-w-sm space-y-2.5 text-center">
            ...
            <p className="text-sm leading-5 text-neutral-500">{AUDIT_LOGS_TEXT.detailPlaceholderDescription}</p>
          </div>
        </CardContent>
      </Card>
    );
      ...
      <CardContent className="space-y-3 overflow-y-auto p-4 xl:flex-1">
        ...
        <div className="grid gap-2.5 md:grid-cols-2">
          <div className="rounded-xl border border-neutral-200 bg-neutral-50 px-3 py-2.5">
            ...
            <p className="mt-1.5 text-xl font-semibold tracking-tight text-neutral-950">{countAuditChanges(selectedLog)}</p>
          </div>
          <div className="rounded-xl border border-neutral-200 bg-neutral-50 px-3 py-2.5">
            <p className="text-[11px] tracking-[0.18em] text-neutral-500 uppercase">Context entries</p>
            <p className="mt-1.5 text-xl font-semibold tracking-tight text-neutral-950">{countAuditContextEntries(selectedLog)}</p>
          </div>
        </div>
