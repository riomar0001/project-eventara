...
function WorkerHealthRow({ entry }: { entry: WorkerHealthEntrySchema }) {
  ...
  return (
    <div className="rounded-xl border border-neutral-200 bg-white/90 p-3 shadow-[0_18px_45px_-38px_rgba(15,23,42,0.28)]">
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 text-xs xl:grid-cols-5">
        <div className="rounded-xl border border-neutral-200/80 bg-neutral-50 px-2.5 py-2.5">
      ...
      <div className="mt-3 rounded-xl border border-dashed border-neutral-200 bg-neutral-50/70 px-3 py-2">
  ...
  return (
    <Card className="border-0 bg-white shadow-none ring-1 ring-neutral-200 xl:flex xl:min-h-[620px] xl:flex-col">
      ...
      <CardContent className="space-y-4 pt-4 xl:flex xl:min-h-0 xl:flex-1 xl:flex-col xl:overflow-hidden">
        {statsError ? (
          <div className="flex h-full min-h-52 flex-col items-center justify-center gap-3 rounded-xl border border-dashed border-rose-200 bg-rose-50/60 px-6 py-10 text-center">
          ...
          <>
            <div className="grid gap-3 md:grid-cols-2 2xl:grid-cols-4">
              {isLoading ? (
                Array.from({ length: 4 }, (_, index) => <Skeleton key={`queue-stat-${index}`} className="h-24 rounded-xl" />)
              ) : (
                <>
            ...
            <div className="rounded-[28px] border border-neutral-200 bg-neutral-50/70 p-3 xl:flex xl:min-h-0 xl:flex-1 xl:flex-col xl:overflow-hidden">
              ...
              <ScrollArea className="mt-3 xl:min-h-0 xl:flex-1">
                <div className="space-y-3 pr-3 pb-3">
                  {isLoading ? (
                    Array.from({ length: 3 }, (_, index) => <Skeleton key={`queue-worker-${index}`} className="h-32 rounded-xl" />)
                  ) : stats?.worker_health.length ? (
                    stats.worker_health.map((entry, index) => <WorkerHealthRow key={`${entry.raw}-${index}`} entry={entry} />)
                  ) : (
                    <div className="flex min-h-44 items-center justify-center rounded-xl border border-dashed border-neutral-200 bg-white/80 px-6 text-center">
                      <div className="space-y-1">
